...
//
//    equality:
//      a ^ b == ((a & b) * 3 + (a | b) * 2 + (a & ~b) * 7 + (~a & b) * 5) * 13 + 17
//
// USAGE:
...
struct MBAXor : public llvm::PassInfoMixin<MBAXor> {
...
//在 LLVM 中，BasicBlock（基本块）是一个基本的控制流单元，它包含一系列按照程序的控制流路径顺序排列的指令。因此，在 LLVM 中，将基本块中的指令作为迭代器的元素是很自然的选择，因为基本块实际上就是一个指令序列
  ...
  //IE = BB.end(): 这里初始化了另一个名为 IE 的迭代器，指向基本块中指令的结束位置
    ...
    IRBuilder<> Builder(BinOp);//创建了一个 IRBuilder 对象,插入新的指令、操作数等到当前基本块所需的
     auto Val3 = ConstantInt::get(BinOp->getType(), 3);
    auto Val2 = ConstantInt::get(BinOp->getType(), 2);
    auto Val7 = ConstantInt::get(BinOp->getType(), 7);
    auto Val5 = ConstantInt::get(BinOp->getType(), 5);
    auto Val13 = ConstantInt::get(BinOp->getType(), 13);
    auto Val17 = ConstantInt::get(BinOp->getType(), 17);

    // Build an instruction representing
    // ((a & b) * 3 + (a | b) * 2 + (a & ~b) * 7 + (~a & b) * 5) * 13 + 17
    Instruction *NewInst =
      // E = e6 + 17
        ...
        Builder.CreateMul(
          ...
          Builder.CreateAdd(
            ...
            Builder.CreateAdd(
              ...
              Builder.CreateMul(
                Val2,
                Builder.CreateOr(BinOp->getOperand(0),
                         BinOp->getOperand(1))
              )
            ),
            // e2 = (a & ~b) * 7
...
//--------------
PreservedAnalyses MBAXor::run(llvm::Function &F,
...
llvmGetPassPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "mba-xor", LLVM_VERSION_STRING,//llvm版本号环境取值,第二个是插件名字,第三个是插件版本号都是随便传
          [](PassBuilder &PB) {   //第四个参数比较重要,回调函数传递PassBuilder &PB对象
            PB.registerPipelineParsingCallback(   
                [](StringRef Name, FunctionPassManager &FPM,
                  ...
                  if (Name == "mba-xor") {
                    FPM.addPass(MBAXor());
                     //FPM.addPass(MBAXor()) 这行代码向函数级别 Pass 管理器 FPM 中添加了一个 MBAXor Pass 的实例。在这里，MBAXor() 表示创建了 MBAXor 结构体的一个实例(默认构造函数)，并将其添加到 Pass 管理器中
                    return true;
                  }
